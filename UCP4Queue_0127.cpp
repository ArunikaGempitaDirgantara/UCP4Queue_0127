...
public:
    ...
    void insert()
    {
        int num; // 1. Allocate memory for the new node.
        int queue_array;
        cout << "Enter a number: "; 
        cin >> num; // 2. Assign value to the data field of the new node.
        cout << endl;

        if (( front == 0 && rear == 0) || (front == rear + 1)) // 3. Make the next field of the new node point to NULL.
        {
            cout << "\nQueue is Empty\n";
            return; // d. Exit
        }
        // 4. If the queue is empty
        if (front == 0) // a. Make FRONT point to the new node
        {
            front = 0;
            rear = 0;
        }
            ...
            if (rear == NULL)  // b. Make REAR  point to the new node
                rear = 0;
            else
    ...
    { 
        int queue_array;
        if (front == NULL) //1. If the queue is empty: // FRONT = NULL
        {
            cout << "Queue empty\n";// a. Display “Queue empty”
            return; // b. Exit
        }
